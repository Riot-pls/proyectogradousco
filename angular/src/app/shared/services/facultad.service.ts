...
import { Facultad } from 'src/app/shared/models/facultad.model';
...
export class FacultadService {
  ...
  actualizarFacultad(facultad:Facultad){
   // console.log(usuario._id);
    let url = this.siteUrl+'/facultad/'+facultad._id+'?token='+this.token;
  ...
  cargarFacultades(desde:number=0){
    ...
    let url=this.siteUrl+'/facultad?desde='+desde;
  ...
  buscarFacultades(termino:string){
    ...
    let url=this.siteUrl+'/busqueda/coleccion/facultad/'+termino;
  ...
  borrarFacultades(id:string){
    ...
    let url=this.siteUrl+'/facultad/'+id+'?token='+this.token;
