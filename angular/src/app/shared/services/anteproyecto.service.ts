...
import { AnteProyecto } from 'src/app/shared/models/anteproyecto.model';
...
export class AnteProyectoService {
  ...
  crearAnteProyecto(anteProyecto: AnteProyecto) {
    const url = this.siteUrl + '/anteProyecto';
  ...
  actualizarAnteProyecto(anteProyecto: AnteProyecto) {
    const url = this.siteUrl + '/anteProyecto/' + anteProyecto._id + '?token=' + this.token;
  ...
  cargarAnteProyectos(desde: number= 0) {
    const url = this.siteUrl + '/anteProyecto?desde=' + desde;
  ...
  */
  ...
  borrarAnteProyectos(id: string) {
    const url = this.siteUrl + '/anteProyecto/' + id + '?token=' + this.token;
